...
public class ViewCommand extends Command {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (replyContent.length() < 1950) { // Discord limit is 2000, leave some buffer
            ...
            // If the combined content is too long, send humanized data as file and EPA in
            // message
            event.reply(epa) // Send EPA data first
                    .addFiles(
                            FileUpload.fromData(s.getBytes(StandardCharsets.UTF_8), "turbo-view-" + teamNumber + ".md")) // Then
                                                                                                                         // attach
                                                                                                                         // the
                                                                                                                         // file
                    .queue();
            ...
            // event.replyFiles(
            // FileUpload.fromData(s.getBytes(StandardCharsets.UTF_8), "turbo-view-" +
            // teamNumber + ".md"),
            // FileUpload.fromData(epa.getBytes(StandardCharsets.UTF_8), "turbo-epa-" +
            // teamNumber + ".txt")
            // ).queue();
        }

        // This code looks like it was written with AI.
        // Consider sending files after the initial reply confirmation for better user
        // experience
        ...
        // );
        // } catch (Exception e) {
        // System.err.println("Error sending file with ID " + fileId + ": " +
        // e.getMessage());
        // // Maybe notify user on Discord? event.getChannel().sendMessage("Failed to
        // send file: " + fileId).queue();
        // }
        // });
    }
    ...
    public static String getEPA(int teamNum) throws IOException { // Added throws IOException
        String usersURL = "https://api.statbotics.io/v3/team_year/" + teamNum + "/2025"; // Still hardcoded 2025 -
        ...
        try {
            ...
            if (responseCode != 200) {
                ...
                try (InputStreamReader errIn = new InputStreamReader(conn.getErrorStream(), StandardCharsets.UTF_8);
                    ...
                    /* Ignore if error stream is unavailable */ }
            ...
            // System.out.println("Statbotics Response: " + completeJsonResponse); // Keep
            // for debugging if needed

            if (!completeJsonResponse.isEmpty()) {
                Gson gson = new Gson();
                try {
                    StatboticsTeamYearResponse stats = gson.fromJson(completeJsonResponse,
                            StatboticsTeamYearResponse.class);
            ...
            if ("strategy".equals(entry.getType()) && (filesObj = entry.getData().get("files")) != null) {
                ...
                if (filesObj instanceof String) {
                    ...
                    try {
                        ...
                        System.err.println(
                                "Warning: 'files' field in strategy entry is not a String or Collection of Strings.");
                    }
                }
